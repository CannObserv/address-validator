...
| `src/address_validator/services/validation/cache_db.py` | `AsyncEngine` singleton; `init_engine()` creates engine + runs `alembic upgrade head` once at startup (called from lifespan); `get_engine()` is a sync accessor that raises if init wasn't called — schema changes go through `alembic/versions/` migrations, not inline DDL |
