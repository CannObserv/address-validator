...
async def test_get_audit_rows_by_raw_input(db: AsyncEngine) -> None:
    """raw_input filter matches the denormalized audit_log.raw_input column directly."""
    await _seed_rows(db)

    # Denormalized: raw_input lives on audit_log itself, no query_patterns join.
    async with db.begin() as conn:
        await conn.execute(
            text("""
                UPDATE audit_log SET raw_input = :raw
                WHERE id = (
...
async def test_get_audit_rows_raw_input_not_set_excluded(db: AsyncEngine) -> None:
    """Rows with NULL raw_input are excluded when filtering by raw_input."""
    await _seed_rows(db)
