...
def test_candidates_badge_renders_count(client: TestClient, admin_headers: dict) -> None:
    with patch(
        "address_validator.routers.admin.candidates.get_new_candidate_count",
        new=AsyncMock(return_value=7),
    ):
        r = client.get("/admin/candidates/_badge", headers=admin_headers)
    assert r.status_code == 200
    assert "7" in r.text
...
def test_candidates_badge_path_does_not_collide_with_detail(
    client: TestClient, admin_headers: dict
) -> None:
    """`_badge` must hit the badge handler, not the `{raw_hash}` detail handler."""
    badge_mock = AsyncMock(return_value=3)
    ...
    with (
        ...
        r = client.get("/admin/candidates/_badge", headers=admin_headers)
    assert r.status_code == 200
    badge_mock.assert_awaited_once()
